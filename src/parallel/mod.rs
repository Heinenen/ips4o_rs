...
use crate::{
    ...
    constants::{BASE_CASE_SIZE, BLOCK_SIZE, MAX_BUCKETS, SINGLE_LEVEL_THRESHOLD, ENABLE_ANALYZER},
    is_less_to_compare,
    parallel::empty_block_movement::move_empty_blocks,
    permute_blocks::permute_blocks_parallel,
    sequential::{calculate_bucket_boundaries, get_splitters, sequential},
    storage::{GlobalStorage, LocalStorage},
    util::{round_up_to_block_size, test_block_permutation, test_cleanup_margins},
    Less, PLess, PSortable, Sortable, crum_analyze::analyze,
};
...
pub(crate) fn parallel<'a, T, F>(
    v: &mut [T],
    lss: &mut [LocalStorage<'a, T, F>],
    ...
    F: PLess<T>,
{
    ...
    par_recurse(v, lss, gs, false, is_less);
}
    ...
    F: PLess<T>,
{
    ...
    partition(v, lss, gs, unbalanced, is_less);
    ...
    for bucket in parallel_queue {
        par_recurse(bucket, lss, gs, true, is_less);
    }
    let lss = lss
        .iter_mut()
        .map(Mutex::new)
...
pub(crate) fn seq_recurse_wrapper<'a, T, F>(
    v: &mut [T],
    lss: &[Mutex<&mut LocalStorage<'a, T, F>>],
    is_less: &'a F,
) where
    T: Sortable,
    F: Less<T>,
{
    ...
    if ENABLE_ANALYZER {
        analyze(v, Some(&mut ls), &is_less);
    } else {
        sequential(v, *ls, is_less);
    }
}
    ...
    F: PLess<T>,
{
    ...
    get_splitters(v, gs, unbalanced, &mut sorting_callback, is_less);

    gs.classifier.build();
    debug_assert!(gs.classifier.test_classification(v, is_less));

    // why is 0.5 added in c++ version?
    ...
    F: Less<T>,
{
    ...
    'block: {
        ...
        // TODO maybe replace with iter.windows(2)

...
use crum_analyze::{analyze, parallel_analyze};
...
use crate::{constants::ENABLE_ANALYZER, base_case::sort_simple_cases};

mod base_case;
mod bucket_pointers;
mod classifier;
mod constants;
pub mod crum_analyze;
mod parallel;
mod permute_blocks;
mod sequential;
mod storage;
...
pub trait Sortable: Clone + Debug + Default + Copy {}
impl<T: Clone + Debug + Default + Copy + Ord> Sortable for T {}

pub trait Less<T>: Fn(&T, &T) -> bool {}
impl<T, F: Fn(&T, &T) -> bool> Less<T> for F {}

pub trait PSortable: Sortable + Send + Sync + Copy {}
impl<T: Sortable + Send + Sync + Copy> PSortable for T {}

pub trait PLess<T>: Less<T> + Sync {}
impl<T, F: Less<T> + Sync> PLess<T> for F {}

#[inline]
pub fn sort<T>(v: &mut [T])
where
    T: Ord + Debug + Default + Clone + Copy,
{
    ips4o(v, T::lt);
    ...
    F: Less<T>,
{
    ...
    // Initializing data structures is not worth it for small inputs.
    if v.len() <= BASE_CASE_MULTIPLIER * BASE_CASE_SIZE {
        base_case::base_case_sort(v, &is_less);
        return;
    }
    if ENABLE_ANALYZER {
        analyze(v, None, &is_less);
    } else {
...
mod tests {
    ...
    use crate::{
        crum_analyze::glidesort_merge::par_double_merge, debug, sort, sort_par, PSortable,
    };

    const TEST_PARALLEL: bool = true;
    ...
    #[test]
    fn pattern_test() {
        let len = 100_000_000 / 2;
        let mut scratches: [Box<[MaybeUninit<i32>; 131072]>; 12] =
            core::array::from_fn(|_| Box::new(unsafe { MaybeUninit::uninit().assume_init() }));
        let mut v = vec![0_i32; len];
        v.iter_mut().enumerate().for_each(|(i, it)| *it = i as i32);
        let mut v_prime = v.clone();
        v.append(&mut v_prime);
        let mut scratches = scratches
            .iter_mut()
            .map(|it| it.as_mut_slice())
            .collect::<Vec<_>>();
        let num_threads = current_num_threads();
        par_double_merge(&mut v, len, &mut scratches[..num_threads], &i32::lt);
    }

    // 2 threads : 11630527ns
    // 6 threads : 17934365ns

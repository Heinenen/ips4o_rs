...
use super::glidesort_merge;

fn min<'a, T: Sortable, F: Less<T>>(left: &'a T, right: &'a T, is_less: &F) -> &'a T {
...
fn max<'a, T: Sortable, F: Less<T>>(left: &'a T, right: &'a T, is_less: &F) -> &'a T {
    if is_less(right, left) {
        left
    } else {
        right
...
fn swap_neighbours<T: Sortable, F: Less<T>>(slice: &mut [T], i: usize, is_less: &F) -> bool {
    let first_greater = is_less(&slice[i + 1], &slice[i]);
...
fn unguarded_insert<T: Sortable, F: Less<T>>(slice: &mut [T], offset: usize, is_less: &F) {
    assert!(offset <= slice.len());

    for i in offset..slice.len() {
        let mut j = i;

        if !is_less(&slice[j], &slice[j - 1]) {
            continue;
        }

        let key = slice[j];

        if is_less(&key, &slice[1]) {
        ...
        swap_neighbours(slice, j, is_less);
    }
}

fn bubble_sort<T: Sortable, F: Less<T>>(slice: &mut [T], is_less: &F) {
    assert!(slice.len() <= 3);

    if slice.len() > 1 {
        if slice.len() > 2 {
            swap_neighbours(slice, 0, is_less);
            swap_neighbours(slice, 1, is_less);
        }
...
fn quad_swap_four<T: Sortable, F: Less<T>>(slice: &mut [T; 4], is_less: &F) {
    swap_neighbours(slice, 0, is_less);
    swap_neighbours(slice, 2, is_less);

    if swap_neighbours(slice, 1, is_less) {
        swap_neighbours(slice, 0, is_less);
        swap_neighbours(slice, 2, is_less);
        swap_neighbours(slice, 1, is_less);
    }
...
impl ParityMerger<true> {
    ...
    pub fn merge<T: Sortable, F: Less<T>>(
        &mut self,
        dst: &mut [T],
        left: &[T],
        right: &[T],
        ...
        let left_less = !is_less(&right[self.right_i], &left[self.left_i]);
    ...
    pub fn merge_last<T: Sortable, F: Less<T>>(
        &self,
        dst: &mut [T],
        left: &[T],
        right: &[T],
        ...
        dst[self.dst_i] = *min(&left[self.left_i], &right[self.right_i], is_less);
    }
}
...
impl ParityMerger<false> {
    ...
    pub fn merge<T: Sortable, F: Less<T>>(
        ...
        let left_greater = is_less(&right[self.right_i], &left[self.left_i]);
    ...
    pub fn merge_last<T: Sortable, F: Less<T>>(
        ...
        dst[self.dst_i] = *max(&left[self.left_i], &right[self.right_i], is_less);
    }
}

#[inline(always)]
fn parity_merge<T: Sortable, F: Less<T>>(dst: &mut [T], src: &[T], is_less: &F) {
    ...
    for _ in 0..half_len - 1 {
        start_merger.merge(dst, left, right, is_less);
        end_merger.merge(dst, left, right, is_less);
    }

    start_merger.merge_last(dst, left, right, is_less);
    end_merger.merge_last(dst, left, right, is_less);
}

fn parity_swap_eight<T: Sortable, F: Less<T>>(slice: &mut [T; 8], is_less: &F) {
    let mut swap = [T::default(); 8];

    swap_neighbours(slice, 0, is_less);
    swap_neighbours(slice, 2, is_less);
    swap_neighbours(slice, 4, is_less);
    swap_neighbours(slice, 6, is_less);

    if !is_less(&slice[2], &slice[1])
        && !is_less(&slice[4], &slice[3])
        && !is_less(&slice[6], &slice[5])
    {
        return;
    }

    parity_merge(&mut swap[..4], &slice[..4], is_less);
    parity_merge(&mut swap[4..], &slice[4..], is_less);

    parity_merge(slice, &swap, is_less);
}

fn parity_swap_sixteen<T: Sortable, F: Less<T>>(slice: &mut [T; 16], is_less: &F) {
    let mut swap = [T::default(); 16];

    quad_swap_four((&mut slice[0..4]).try_into().unwrap(), is_less);
    quad_swap_four((&mut slice[4..8]).try_into().unwrap(), is_less);
    quad_swap_four((&mut slice[8..12]).try_into().unwrap(), is_less);
    quad_swap_four((&mut slice[12..16]).try_into().unwrap(), is_less);

    if !is_less(&slice[4], &slice[3])
        && !is_less(&slice[8], &slice[7])
        && !is_less(&slice[12], &slice[11])
    {
        return;
    }

    parity_merge(&mut swap[..8], &slice[..8], is_less);
    parity_merge(&mut swap[8..], &slice[8..], is_less);
...
pub(crate) fn tail_swap<T: Sortable, F: Less<T>>(slice: &mut [T], is_less: &F) {
    assert!(slice.len() < 32);

    if let Some(head) = get_array(slice) {
        parity_swap_sixteen(head, is_less);
        unguarded_insert(slice, 16, is_less);
    } else if let Some(head) = get_array(slice) {
        parity_swap_eight(head, is_less);
        unguarded_insert(slice, 8, is_less);
    } else if let Some(head) = get_array(slice) {
        quad_swap_four(head, is_less);
        unguarded_insert(slice, 4, is_less);
    } else {
        bubble_sort(slice, is_less);
    }
}

fn parity_tail_swap_eight<T: Sortable, F: Less<T>>(slice: &mut [T; 8], is_less: &F) {
    let mut swap = [T::default(); 8];

    swap_neighbours(slice, 4, is_less);

    if !swap_neighbours(slice, 6, is_less)
        && !is_less(&slice[4], &slice[3])
        && !is_less(&slice[6], &slice[5])
    {
        return;
...
fn parity_tail_flip_eight<T: Sortable, F: Less<T>>(slice: &mut [T; 8], is_less: &F) {
    let mut swap = [T::default(); 8];
...
impl ForwardMerger {
    ...
    pub fn merge_left<T: Sortable, F: Less<T>>(
        &mut self,
        dst: &mut [T],
        left: &[T],
        right: &[T],
        ...
        let left_less = !is_less(&right[self.right_i], &left[self.left_i + 1]);
    ...
    pub fn merge_right<T: Sortable, F: Less<T>>(
        &mut self,
        dst: &mut [T],
        left: &[T],
        right: &[T],
        ...
        let left_greater = is_less(&right[self.right_i + 1], &left[self.left_i]);
...
fn forward_merge<T: Sortable, F: Less<T>>(dst: &mut [T], src: &[T], is_less: &F) {
    assert!(dst.len() >= 4);
    assert_eq!(dst.len(), src.len());
    ...
    if !is_less(&right[half_len - 1 - half_len / 4], &left[half_len - 1]) {
        while merger.left_i < half_len - 2 {
            if !merger.merge_left(dst, left, right, is_less)
                && !merger.merge_right(dst, left, right, is_less)
            {
                merger.merge(dst, left, right, is_less);
            }
        }

        while merger.left_i < half_len {
            merger.merge_last(dst, left, right, is_less);
        }

        while merger.right_i < half_len {
            merger.copy_from::<_, false>(dst, right);
        }
    } else if is_less(&right[half_len - 1], &left[half_len - 1 - half_len / 4]) {
        while merger.right_i < half_len - 2 {
            if !merger.merge_right(dst, left, right, is_less)
                && !merger.merge_left(dst, left, right, is_less)
            {
                merger.merge(dst, left, right, is_less);
            }
        }
        ...
        parity_merge(dst, src, is_less);
    }
}

fn quad_merge_block<T: Sortable, F: Less<T>>(slice: &mut [T], swap: &mut [T], is_less: &F) {
    assert!(slice.len() >= swap.len());

    let quarter_len = swap.len() / 4;

    if !is_less(&slice[quarter_len], &slice[quarter_len - 1]) {
        if !is_less(&slice[3 * quarter_len], &slice[3 * quarter_len - 1]) {
            if !is_less(&slice[2 * quarter_len], &slice[2 * quarter_len - 1]) {
                return;
            }

            swap.copy_from_slice(&slice[..4 * quarter_len]);

            return forward_merge(&mut slice[..4 * quarter_len], swap, is_less);
        }

        swap[..2 * quarter_len].copy_from_slice(&slice[..2 * quarter_len]);
    } else {
        forward_merge(
            &mut swap[..2 * quarter_len],
            &slice[..2 * quarter_len],
            is_less,
        );
    }
    ...
    forward_merge(&mut slice[..4 * quarter_len], swap, is_less);
}
...
impl PartialForwardMerger {
    ...
    pub fn merge<T: Sortable, F: Less<T>>(&mut self, slice: &mut [T], swap: &[T], is_less: &F) {
        if is_less(&slice[self.right_i + 1], &swap[self.swap_i]) {
            self.right_to_start(slice);
        } else if !is_less(&slice[self.right_i], &swap[self.swap_i + 1]) {
            self.swap_to_start(slice, swap);
        } else {
            let swap_less = !is_less(&slice[self.right_i], &swap[self.swap_i]);
...
fn partial_forward_merge<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    mut swap: &mut [T],
    block_len: usize,
    is_less: &F,
    ...
    while merger.swap_i < block_len - 2 && merger.right_i < slice.len() - 2 {
        merger.merge(slice, swap, is_less);
    }
    ...
    let needs_copy = loop {
        ...
        let swap_less = !is_less(&right_val, &swap_val);
        let right_less = !swap_less;

        slice[merger.start_i] = *min(&swap_val, &right_val, is_less);
...
impl PartialBackwardMerger {
    ...
    pub fn merge<T: Sortable, F: Less<T>>(&mut self, slice: &mut [T], swap: &[T], is_less: &F) {
        if is_less(&swap[self.swap_i], &slice[self.mid_i - 1]) {
            self.mid_to_end(slice);
        } else if !is_less(&swap[self.swap_i - 1], &slice[self.mid_i]) {
            self.swap_to_end(slice, swap);
        } else {
            let mid_less = !is_less(&swap[self.swap_i], &slice[self.mid_i]);
...
fn partial_backward_merge<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    swap: &mut [T],
    block_len: usize,
    is_less: &F,
    ...
    if !is_less(&slice[block_len], &slice[block_len - 1]) {
        return;
    }
    ...
    let needs_copy = loop {
        let mid_greater = is_less(&swap[merger.swap_i], &slice[merger.mid_i]);
        let swap_greater = !mid_greater;

        slice[merger.end_i] = *max(&slice[merger.mid_i], &swap[merger.swap_i], is_less);
...
fn tail_merge<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    swap: &mut [T],
    mut block_len: usize,
    is_less: &F,
    ...
    while block_len < slice.len() && block_len <= swap.len() {
        for chunk in slice.chunks_mut(2 * block_len) {
            if chunk.len() > block_len {
                partial_backward_merge(chunk, swap, block_len, is_less);
            }
        }
...
fn quad_merge<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    swap: &mut [T],
    mut block_len: usize,
    is_less: &F,
    ...
    while block_len <= slice.len() && block_len <= swap.len() {
        ...
        for chunk in chunks.by_ref() {
            quad_merge_block(chunk, &mut swap[..block_len], is_less);
        }

        tail_merge(chunks.into_remainder(), swap, block_len / 4, is_less);

        block_len *= 4;
    }

    tail_merge(slice, swap, block_len / 4, is_less);

    block_len / 2
}

fn monobound_binary_first<T: Sortable, F: Less<T>>(
    slice: &[T],
    val: T,
    mut top: usize,
    is_less: &F,
...
#[allow(unused)]
pub(crate) fn blit_merge_block<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    swap: &mut [T],
    ...
    if !is_less(&slice[left_block_len], &slice[left_block_len - 1]) {
        return;
    }
    ...
    if left > 0 {
        ...
        if left <= swap.len() {
            ...
            blit_merge_block(slice, swap, left_block_len, left, is_less);
        }
    }
...
fn blit_merge<T: Sortable, F: Less<T>>(
    slice: &mut [T],
    swap: &mut [T],
    mut block_len: usize,
    is_less: &F,
    ...
    while block_len < slice.len() {
        for chunk in slice.chunks_mut(2 * block_len) {
            if chunk.len() > block_len {
                let maybe_uninit_swap =
                    unsafe { &mut *(swap as *mut [T] as *mut [MaybeUninit<T>]) };
                glidesort_merge::double_merge(chunk, block_len, maybe_uninit_swap, is_less);
            }
        }
...
fn swap_four<T: Sortable, F: Less<T>>(slice: &mut [T; 4], is_less: &F) -> bool {
    match (
        is_less(&slice[3], &slice[2]),
        is_less(&slice[2], &slice[1]),
        is_less(&slice[1], &slice[0]),
    ) {
...
fn quad_swap<T: Sortable, F: Less<T>>(slice: &mut [T], is_less: &F) -> bool {
    ...
    while count > 0 {
        ...
        if !swap_four((&mut slice[i..i + 4]).try_into().unwrap(), is_less) {
            count -= 1;

            parity_tail_swap_eight((&mut slice[i..i + 8]).try_into().unwrap(), is_less);
        ...
        let count_not_zero = loop {
            ...
            if let Some(new_count) = new_count {
                ...
                if is_less(&slice[i + 1], &slice[i]) {
                    if is_less(&slice[i + 3], &slice[i + 2]) {
                        if is_less(&slice[i + 2], &slice[i + 1])
                            && is_less(&slice[i], &slice[i - 1])
                        {
                            continue;
                        }
                    ...
                    slice.swap(i, i + 1);
                } else if is_less(&slice[i + 3], &slice[i + 2]) {
                    slice.swap(i + 2, i + 3);
                }
            }
        ...
        if count_not_zero {
            if is_less(&slice[i + 2], &slice[i + 1]) {
                ...
                swap_neighbours(slice, i, is_less);
                swap_neighbours(slice, i + 2, is_less);
                swap_neighbours(slice, i + 1, is_less);
            }
            ...
            if count % 2 == 0 {
                ...
                parity_tail_flip_eight((&mut slice[i..i + 8]).try_into().unwrap(), is_less);
            } else {
                count -= 1;
        ...
        if start_reverse == 0 {
            let is_reversed = !slice[i - 1..(i + 7).min(slice.len())]
                .windows(2)
                .any(|window| !is_less(&window[1], &window[0]));
    ...
    tail_swap(&mut slice[i..(i + 8).min(len)], is_less);

    let mut chunks = slice.chunks_exact_mut(32);

    for chunk in chunks.by_ref() {
        if !is_less(&chunk[8], &chunk[7])
            && !is_less(&chunk[16], &chunk[15])
            && !is_less(&chunk[24], &chunk[23])
        {
            continue;
        }

        parity_merge(&mut swap[..16], &chunk[..16], is_less);
        parity_merge(&mut swap[16..], &chunk[16..], is_less);

        parity_merge(chunk, &swap, is_less);
    }

    let tail = chunks.into_remainder();

    if !tail.is_empty() {
        tail_merge(tail, swap.as_mut_slice(), 8, is_less);
    }

    false
}

pub(crate) fn quadsort_swap<T: Sortable, F: Less<T>>(slice: &mut [T], swap: &mut [T], is_less: &F) {
    if slice.len() < 32 {
        tail_swap(slice, is_less);
    } else if !quad_swap(slice, is_less) {
        let block_len = quad_merge(slice, swap, 32, is_less);
        blit_merge(slice, swap, block_len, is_less);
    }
}
...
mod tests {
    ...
    fn unguarded_insert3() {
        ...
        unguarded_insert(&mut slice, 4, &i32::lt);
    ...
    fn unguarded_insert5() {
        ...
        unguarded_insert(&mut slice, 2, &i32::lt);
    ...
    fn quad_swap_four_swapped() {
        ...
        quad_swap_four(&mut slice, &i32::lt);
    ...
    fn parity_merge_zigzag() {
        ...
        parity_merge(&mut dst, &src, &i32::lt);
    ...
    fn parity_swap_eight_scrambled() {
        ...
        parity_swap_eight(&mut slice, &i32::lt);
    ...
    fn parity_swap_sixteen_scrambled() {
        ...
        parity_swap_sixteen(&mut slice, &i32::lt);
    ...
    fn parity_tail_swap_eight_scrambled() {
        ...
        parity_tail_swap_eight(&mut slice, &i32::lt);
    ...
    fn parity_tail_flip_eight_zigzag() {
        ...
        parity_tail_flip_eight(&mut slice, &i32::lt);
    ...
    fn forward_merge_left() {
        ...
        forward_merge(&mut dst, &src, &i32::lt);
    ...
    fn quad_merge_block_rotated2() {
        ...
        quad_merge_block(&mut slice, &mut [0; 8], &i32::lt);
    ...
    fn partial_forward_merge4() {
        ...
        partial_forward_merge(&mut slice, &mut [0; 4], 4, &i32::lt);
    ...
    fn partial_backward_merge4() {
        ...
        partial_backward_merge(&mut slice, &mut [0; 4], 4, &i32::lt);
    ...
    fn tail_merge2() {
        ...
        tail_merge(&mut slice, &mut [0; 4], 2, &i32::lt);
    ...
    fn quad_merge_scrambled() {
        ...
        assert_eq!(quad_merge(&mut slice, &mut [0; 8], 2, &i32::lt), 16);
    ...
    fn monobound_binary_first_search() {
        ...
        for val in slice {
            assert_eq!(
                monobound_binary_first(&slice, val, slice.len(), &usize::lt),
                val - 1
            );
        }
    }
    ...
    fn blit_merge_block4_left() {
        ...
        blit_merge_block(&mut slice, &mut [0; 4], 4, 4, &i32::lt);
    ...
    fn blit_merge_block8_left() {
        ...
        blit_merge_block(&mut slice, &mut [0; 4], 8, 8, &i32::lt);
    ...
    fn blit_merge_block10_left() {
        ...
        blit_merge_block(&mut slice, &mut [0; 4], 10, 6, &i32::lt);
    ...
    fn blit_merge_scrambled() {
        ...
        blit_merge(&mut slice, &mut [0; 8], 4, &i32::lt);
    ...
    fn quad_swap_all_swaps() {
        ...
        assert!(!quad_swap(&mut slice, &i32::lt));
